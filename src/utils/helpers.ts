...
export function validateDuplicateItems(list: FormattedPlaylistItem[]): {
    uniqueItems: FormattedPlaylistItem[];
    duplicateItems: FormattedPlaylistItem[];
} {
    ...
    list.forEach((item) => {
        ...
        if (
            !uniqueItems.some(
                (i) =>
                    i.videoId === videoId || (i.title === title && i.videoOwnerChannelTitle === videoOwnerChannelTitle),
            )
        ) {
